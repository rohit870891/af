...
async def start_clone_bot(FwdBot, data=None):
    try:
        await asyncio.wait_for(FwdBot.start(), timeout=15)
    except asyncio.TimeoutError:
        raise Exception("❌ Bot start timeout (session/token issue)")
...
async def update_configs(user_id, key, value):
  ...
  if key in ['caption', 'duplicate', 'db_uri', 'forward_tag', 'protect', 'file_size', 'size_limit', 'extension', 'keywords', 'button', 'auto_fwd']:
     current[key] = value
  else: 
